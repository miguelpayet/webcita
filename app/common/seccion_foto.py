...
def foto(view_name, cur_language, secciones):
    ...
    try:
        txt = sec_foto.textoseccionfoto_set.get(idioma__codigo=cur_language)
    ...
    dict_foto = {'clase': clase, 'filas': arr_filas, 'nombre': sec_foto.nombre, 'posicion': sec_foto.posicion,
                 'seccion': 'app/seccion_foto.html', 'titulo': txt.titulo if txt else ''}
...
def obtener_fotos(fila, cur_language, total):
    ...
    for f in qs_fotos:
        try:
            txt = f.textofotofila_set.get(idioma__codigo=cur_language)
