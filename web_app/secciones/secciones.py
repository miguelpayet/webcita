...
from web_app.models.SeccionContacto import TextoContacto
...
def contacto(idioma):
    ...
    list_social = social()
    return {'direccion': contact.direccion, 'estilo': contact.clase, 'mapa': contact.mapa, 'mail': contact.mail,
            'social': list_social,
...
def idiomas(cur_language):
    ...
    try:
        ...
        idioma = Idioma.objects.get(codigo=settings.LANGUAGE_CODE)
    if not idioma:
        raise Exception("no existe idioma %s" % cur_language)
    arr_idioma.append({'nombre': idioma.nombre, 'codigo': idioma.codigo})
...
def parametros(idioma):
    ...
    return {'copyright': txt.copyright, 'logo_inferior': txt.parametro.logomarron,
            'logo_superior': txt.parametro.logoblanco,
...
def texto_foto(pagina, idioma, secciones):
    for seccion in pagina.secciontextofoto_set.all():
        ...
        secciones.append(
            {'estilo': estilo, 'imagen': seccion.imagen, 'nombre': seccion.nombre, 'posicion': seccion.posicion,
